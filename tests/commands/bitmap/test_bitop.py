...
def test_bitop_not_with_multiple_keys(redis: Redis):
    ...
    with pytest.raises(
        Exception, match='The "NOT" operation takes only one source key'
    ):
...
def test_bitop_diff_requires_multiple_keys(redis: Redis):
    ...
    with pytest.raises(
        Exception, match="BITOP DIFF must be called with at least two source keys"
    ):
...
def test_bitop_diff1_requires_multiple_keys(redis: Redis):
    ...
    with pytest.raises(
        Exception, match="BITOP DIFF1 must be called with at least two source keys"
    ):
...
def test_bitop_andor_requires_multiple_keys(redis: Redis):
    ...
    with pytest.raises(
        Exception, match="BITOP ANDOR must be called with at least two source keys"
    ):
