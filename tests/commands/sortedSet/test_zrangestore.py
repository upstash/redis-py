...
def test_zrangestore(redis: Redis):
    ...
    result = redis.zrangestore(destination, sorted_set, min=0, max=2)
    assert result == 3
...
def test_zrangestore_rev(redis: Redis):
    ...
    result = redis.zrangestore(destination, sorted_set, min=0, max=2, rev=True)
    assert result == 3
...
def test_zrangestore_limit(redis: Redis):
    ...
    result = redis.zrangestore(destination, sorted_set, min=0, max=2, offset=1, count=1)
    assert result == 1
