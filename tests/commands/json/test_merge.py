...
def test_merge(redis: Redis):
    ...
    assert redis.json.merge(key, '$', {'str': 'test', 'int': 2}) == True
    assert redis.json.get(key) == [{"int": 2, "array": [1, 2, 3, 4], "object": {"array": [1, 2, 3]}, "str": "test"}]
...
def test_merge_wildcard(redis: Redis):
    ...
    assert redis.json.merge(key, path, [2, 2, 3, 4]) == True
    assert redis.json.get(key) == [{"int": 1, "array": [2, 2, 3, 4], "object": {"array": [2, 2, 3, 4]}}]
