...
def test_mset(redis: Redis):
    ...
    assert redis.json.mset([(json_key_1, "$", value1), (json_key_2, "$", value2)]) == True
    assert redis.json.mget([json_key_1, json_key_2], "$.int") == [[1], [2]]
    assert redis.json.mset([(json_key_1, "$.int", 2), (json_key_2, "$.int", 3)]) == True
    assert redis.json.mget([json_key_1, json_key_2], "$.int") == [[2], [3]]
    assert redis.json.mset([(json_key_1, "$..array[0]", 2), (json_key_2, "$..array[0]", 3)]) == True
