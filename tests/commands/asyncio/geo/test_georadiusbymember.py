...
async def test() -> None:
    async with redis:
        assert await redis.georadiusbymember("test_geo_index", "Catania", 200, "KM") == ["Palermo", "Catania"]
...
async def test_with_count() -> None:
    async with redis:
        assert await redis.georadiusbymember("test_geo_index", "Catania", 200, unit="KM", count=1) == ["Catania"]
