...
async def test() -> None:
    async with redis:
        ...
        assert await redis.geosearchstore(
            "geosearchstore_georadius_dist", "test_geo_index", fromlonlat_longitude=15, fromlonlat_latitude=37, byradius=200, unit="KM",
        ) == 2
...
async def test_with_box() -> None:
    async with redis:
        await redis.delete("geosearchstore_georadius_dist")
        assert await redis.geosearchstore(
            "geosearchstore_georadius_dist", "test_geo_index", fromlonlat_longitude=14, fromlonlat_latitude=35, bybox_height=600, bybox_width=4000, unit="km"
        ) == 1
...
async def test_with_count() -> None:
    async with redis:
        await redis.delete("geosearchstore_georadius_dist")
        assert await redis.geosearchstore(
            "geosearchstore_georadius_dist", "test_geo_index", fromlonlat_longitude=15, fromlonlat_latitude=37, byradius=200, unit="KM", count=1
        ) == 1
        assert await redis.zcard("geosearchstore_georadius_dist") == 1
